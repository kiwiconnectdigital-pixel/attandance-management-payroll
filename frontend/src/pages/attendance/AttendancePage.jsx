...
export default function AttendancePage() {
  ...
  const [captureMode, setCaptureMode] = useState(null); // 'checkin' | 'checkout'
  ...
  // Derived today values from records
  const todayStr = now.toISOString().split('T')[0];
  const todayRecord = records.find((r) => r.date?.startsWith(todayStr));
  const checkInTime = todayRecord?.checkIn?.time
    ? new Date(todayRecord.checkIn.time).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })
    : null;
  const checkOutTime = todayRecord?.checkOut?.time
    ? new Date(todayRecord.checkOut.time).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })
    : null;
  const workedHours = todayRecord?.workingHours
    ...
    <>
      <style>{`
        ...
        .atn-live-dot {
          ...
          animation: atn-pulse 2s infinite;
        }
        ...
        .atn-action-btn.checkin {
          background: linear-gradient(135deg, #16803a, #15803d);
          box-shadow: 0 4px 20px rgba(34,197,94,0.25);
        }
        .atn-action-btn.checkout {
        ...
        .atn-record-card:hover { background: #1a2336; }

        .atn-date-block { flex-shrink: 0; text-align: center; min-width: 40px; }
        .atn-day {
          font-family: 'DM Mono', monospace;
          font-size: 22px; font-weight: 500; line-height: 1; color: #f0f4ff;
        }
        .atn-weekday {
          font-size: 10px; color: #5a6a85; font-weight: 500;
        ...
        .atn-rt-label {
          ...
          letter-spacing: 0.5px; color: #5a6a85; font-weight: 500;
        }
        ...
        .atn-record-hours {
          ...
          font-family: 'DM Mono', monospace;
          font-size: 13px; font-weight: 500; color: #8b9ab5;
        }
        ...
        .atn-modal-title small {
          font-size: 13px; font-weight: 400; color: #8b9ab5;
        ...
        .atn-cam-wrap video,
        .atn-cam-wrap > div:first-child { width: 100% !important; height: 100% !important; object-fit: cover; }
      ...
      <div className="atn-root">
        ...
        <div className="atn-page">
          ...
          <div className="atn-today-card">
            ...
            <div className="atn-status-row">
              <div className="atn-status-pill">
                ...
                <div className={`atn-sp-val ${checkInTime ? 'green' : 'muted'}`}>
                  {checkInTime || '—'}
                </div>
              </div>
              <div className="atn-status-pill">
                <div className="atn-sp-label">Check Out</div>
                <div className={`atn-sp-val ${checkOutTime ? 'green' : 'muted'}`}>
                  {checkOutTime || '—'}
                </div>
              </div>
              <div className="atn-status-pill">
                <div className="atn-sp-label">Hours</div>
                <div className={`atn-sp-val ${workedHours ? 'green' : 'muted'}`}>
                  {workedHours || '—'}
                </div>
              </div>
            </div>
          ...
          <div className="atn-action-row">
            <button
              className="atn-action-btn checkin"
              onClick={() => { setCaptureMode('checkin'); setShowCamera(true); }}
            >
            ...
            <button
              ...
              onClick={() => { setCaptureMode('checkout'); setShowCamera(true); }}
            >
              <span className="atn-btn-icon">✖</span>
              Check Out
          ...
          <div className="atn-records-list">
            {records.length === 0 ? (
              ...
              records.map((r) => {
                ...
                const ciVal = r.checkIn?.time
                  ? new Date(r.checkIn.time).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })
                  : null;
                const coVal = r.checkOut?.time
                  ? new Date(r.checkOut.time).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })
                  : null;
                const badgeLabel =
                ...
                return (
                  <div key={r._id} className="atn-record-card">
                    ...
                    <div className="atn-record-body">
                      ...
                      <div className="atn-record-meta">
                        ...
                        {r.isLate && (
                          <span className="atn-badge late">Late {r.lateByMinutes}m</span>
                        )}
                      </div>
                    </div>
        ...
        >
          <div className="atn-modal-sheet">
            ...
            <div className="atn-cam-wrap">
              ...
              <div className="atn-cam-guide">
                <svg viewBox="0 0 140 160" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <rect x="2" y="2" width="136" height="156" rx="68"
            ...
            <div className="atn-modal-actions">
              ...
              <button className="atn-btn-cancel" onClick={() => setShowCamera(false)}>
                Cancel
              </button>
            </div>
          </div>
