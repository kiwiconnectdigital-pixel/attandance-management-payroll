...
function numberToWords(num) {
  ...
  if (intPart >= 1000)
    result +=
      convertHundreds(Math.floor((intPart % 100000) / 1000)) + " Thousand ";
...
module.exports = {
  generatePayslip: async (req, res, next) => {
    try {
      ...
      doc.font("Helvetica").fontSize(10).text(`E-mail: ${companyEmail}`, L, 76);

      doc.moveTo(L, 95).lineTo(R, 95).stroke();

      const calendarDays =
        payroll.att_calendar_days ||
      ...
      doc
        .moveTo(colMid, tblTop)
        .lineTo(colMid, tblTop + hdrH)
        .stroke();
      doc
        .moveTo(earAmtL, tblTop)
        .lineTo(earAmtL, tblTop + hdrH)
        .stroke();
      doc
        .moveTo(dedAmtL, tblTop)
        .lineTo(dedAmtL, tblTop + hdrH)
        .stroke();
      ...
      const incentive =
        (payroll.earning_bonus || 0) + (payroll.earning_overtime || 0);
      ...
      for (let i = 0; i <= dataRows; i++) {
        doc
          .moveTo(L, tblDataT + i * dataRowH)
          .lineTo(R, tblDataT + i * dataRowH)
          .stroke();
      }
      ...
      earRows.forEach(([label, value], i) => {
        ...
        doc
          ...
          .fontSize(9)
          .text(label, L + 4, y);
        doc.text(String(Math.round(value || 0)), earAmtL + 4, y, {
          width: colMid - earAmtL - 8,
          align: "right",
        });
      });
      ...
      dedRows.forEach(([label, value], i) => {
        ...
        doc
          .font("Helvetica")
          .fontSize(9)
          .text(label, colMid + 4, y);
        doc.text(String(Math.round(value || 0)), dedAmtL + 4, y, {
          width: R - dedAmtL - 8,
          align: "right",
        });
      });
      ...
      doc
        ...
        .fontSize(9)
        .text("Total", L + 4, totY)
        .text(String(Math.round(earTotal || 0)), earAmtL + 4, totY, {
          width: colMid - earAmtL - 8,
          align: "right",
        })
        .text("Net Salary", colMid + 4, totY)
        .text(String(Math.round(net || 0)), dedAmtL + 4, totY, {
          width: R - dedAmtL - 8,
          align: "right",
        });
