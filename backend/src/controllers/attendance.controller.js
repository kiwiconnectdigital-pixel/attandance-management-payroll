...
const { isLate, calculateWorkingHoursFromPunches } = require('../services/attendance.service');
...
if (employeeWithBranch?.branch) {
  ...
  if (!geo.allowed) throw new ApiError(403, geo.message);
    ...
    const lateInfo = isLate(checkInTime);
    ...
    // ── Fix: find or create first, then update — avoids $set/$setOnInsert conflict ──
    ...
    res.json(new ApiResponse(200, attendance,
      `Checked in at ${moment(checkInTime).format('hh:mm A')} ✓ Face verified`
    ));
  } catch (error) {
    next(error);
  }
...
const getTodaySummary = async (req, res, next) => {
  try {
    ...
    const [totalEmployees, presentToday, lateToday] = await Promise.all([
      ...
      Attendance.countDocuments({ date: { $gte: today, $lte: tomorrow }, status: 'present' }),
      Attendance.countDocuments({ date: { $gte: today, $lte: tomorrow }, isLate: true }),
    ]);
...
module.exports = { checkIn, checkOut, getAttendance, getTodaySummary };