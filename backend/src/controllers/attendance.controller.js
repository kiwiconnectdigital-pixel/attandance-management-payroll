...
module.exports = {
  ...
  checkInWithLocation: async (req, res, next) => {
    try {
      const { latitude, longitude, address, accuracy_meters } = req.body;
      ...
      if (company.office_location_enabled === true) {
        ...
        const branchLat = parseFloat(branch.geofence_latitude);

        const branchLng = parseFloat(branch.geofence_longitude);
        ...
        allowedRadius = Number(branch.geofence_radius_meters) || 50;
        ...
        const dLat = toRadians(branchLat - parsedLat);

        const dLng = toRadians(branchLng - parsedLng);
      ...
      const punch = await Punch.create({
        ...
        address: address || branch.geofence_address || "GPS captured",
      ...
      const statusNote = lateInfo.isLate
        ? `Late by ${lateInfo.minutes}m${
            lateInfo.minutes >= 30 ? " — marked half-day" : ""
          }`
        : "On time";
      ...
      res.json(
        new ApiResponse(
          200,
          {
            ...
            geofence: company.office_location_enabled
              ? {
                  enabled: true,
                  branchId: branch.id,
                  branchName: branch.name,
                  distanceMeters: Math.round(distanceFromOffice),
    ...
    try {
      ...
      if (lastPunch) {
        ...
        if (secondsSinceLast < 5) {
          throw new ApiError(
            400,
            "Duplicate check-out detected. Please wait a moment and try again.",
          );
        }
      }
      if (!req.file)
  ...
  getAttendance: async (req, res, next) => {
    try {
      ...
      for (const record of records) {
        ...
        record.dataValues.checkIns = punches
          .filter((p) => p.type === "check_in")
          .map(serializePunch);
        record.dataValues.checkOuts = punches
          .filter((p) => p.type === "check_out")
          .map(serializePunch);
      }
      ...
      const summary = await Attendance.findAll({
        attributes: [
          ...
          [
            sequelize.fn(
              "SUM",
              sequelize.literal("CASE WHEN is_late = 1 THEN 1 ELSE 0 END"),
            ),
            "lateToday",
          ],
          [
  ...
  getAttendanceById: async (req, res, next) => {
    try {
      ...
      attendance.dataValues.checkIns = punches
        .filter((p) => p.type === "check_in")
        .map(serializePunch);
      attendance.dataValues.checkOuts = punches
        .filter((p) => p.type === "check_out")
        .map(serializePunch);
  ...
  updateAttendance: async (req, res, next) => {
    try {
      ...
      if (lateByMinutes !== undefined)
        updateData.late_by_minutes = lateByMinutes;
      ...
      if (checkInTime) {
        ...
        const checkInDateTime = moment
          .tz(attendance.date, "Asia/Kolkata")
      ...
      if (checkInTime || checkOutTime) {
        ...
        if (checkIns.length > 0 && checkOuts.length > 0) {
          ...
          const { workingHours: wh, overtimeHours: oh } =
            calculateWorkingHoursFromPunches(
              checkIns,
              checkOuts,
              workStartHour,
      ...
      res.json(
        new ApiResponse(200, updated, "Attendance updated successfully"),
      );
    } catch (error) {
      next(error);
    }
  ...
  createAttendance: async (req, res, next) => {
    try {
      ...
      res.json(
        new ApiResponse(201, created, "Attendance created successfully"),
      );
    } catch (error) {
      next(error);
    }
    ...
    try {
      ...
      if (status === "absent") {
        ...
        await Attendance.update(
          { working_hours: 4.5 },
          {
            where: {
              ...where,
              [Op.or]: [{ working_hours: 0 }, { working_hours: null }],
            },
          },
        );
      ...
      res.json(
        new ApiResponse(
          200,
          null,
          `Bulk updated ${updatedCount} records to ${status}`,
        ),
      );
    } catch (error) {
      next(error);
  ...
  trackLocation: async (req, res, next) => {
    try {
      ...
      if (!attendance) {
        throw new ApiError(400, "You are not checked in today");
      }
      ...
      if (lastLog) {
        const previousLat = Number(lastLog.latitude);

        const previousLng = Number(lastLog.longitude);

        const previousTime = new Date(lastLog.recorded_at).getTime();

        const currentTime = recorded_at
          ? new Date(recorded_at).getTime()
          : Date.now();

        const elapsedSeconds =
          Math.max(currentTime - previousTime, 1000) / 1000;
        ...
        calculatedSpeed = distanceFromPrevious / elapsedSeconds;

        if (distanceFromPrevious > 1000 && calculatedSpeed > 100) {
          console.warn("[Location] GPS jump rejected", {
          ...
          return res.json(
            new ApiResponse(
              200,
              {
                skipped: true,
                reason: "gps_jump",
                distance: Math.round(distanceFromPrevious),
                calculatedSpeed: Number(calculatedSpeed.toFixed(2)),
              },
      ...
      if (AttendanceLocationLog.rawAttributes?.altitude !== undefined) {
        locationData.altitude = gpsAltitude;
      }

      if (AttendanceLocationLog.rawAttributes?.heading !== undefined) {
        locationData.heading = gpsHeading;
      }

      if (AttendanceLocationLog.rawAttributes?.speed !== undefined) {
        locationData.speed = gpsSpeed !== null ? gpsSpeed : calculatedSpeed;
      }

      if (AttendanceLocationLog.rawAttributes?.is_mocked !== undefined) {
        locationData.is_mocked = Boolean(is_mocked);
      }

      const log = await AttendanceLocationLog.create(locationData);

      const io = req.app.get("io");

      if (io) {
        io.emit("employee-location-update", {
          employeeId: employee.id,
          ...
          speed: gpsSpeed !== null ? gpsSpeed : calculatedSpeed,
  ...
  getLocationTrail: async (req, res, next) => {
    try {
      ...
      const points = logs.map((l) => ({
        ...
        address: l.address,
        accuracy_meters:
          l.accuracy_meters !== null ? parseFloat(l.accuracy_meters) : null,
        source: l.source,
  ...
  getLiveLocations: async (req, res, next) => {
    try {
      ...
      for (const att of attendances) {
        ...
        const checkOutCount = await Punch.count({
          where: { attendance_id: att.id, type: "check_out" },
        });
